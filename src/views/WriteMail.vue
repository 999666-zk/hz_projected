  ...
  >
    ...
    <div class="send-header" style="position: relative">
      ...
      <div class="writeMailHeaderRight">
        <div
          class="writeMailHeaderItem flex-center cursor"
          style="margin-right: 5px; background-color: #3471ff"
          @click="handleSendMail('send')"
        >
          <!-- <img class="writeMailHeaderActionIcon" src="@/assets/img/mail/write-send.png" alt=""/> -->
          <svg
            t="1670492004246"
            class="icon writeMailHeaderActionIcon"
      ...
      <div class="send-tip" style="display: flex">
        <span class="cursor" @click="addRow('cs', showCs)" v-show="!showFbfs"
          >{{ showCs ? "删除" : "添加" }}抄送</span
        >
        <!-- <span class="cursor" @click="addRow('ms', showMs)" v-show="!showFbfs"
          >{{ showMs ? "删除" : "添加" }}密送</span
        > -->
        ...
        <div style="display: flex">
          ...
          @error="onError"
          lineMode
          errorText='请上传正确的附件' style="padding-left:10px;" class="file"/> -->
        </div>
      </div>
    </div>
    <div class="send-main-container">
      ...
      <div class="send-row" v-show="!showFbfs">
        <div class="send-label" style="color: #3471ff">发件人：</div>
        <i
          @click="handleShowAddDialog('to')"
          class="el-input__icon el-icon-circle-plus-outline"
        ></i>
        <div class="send-row-item-container">
          <el-select
            v-model="mailConfigId"
            class="mailselectEl"
            remote
            allow-create
      ...
      <div class="send-tip">
        <div
          class="bigFileListItem"
          v-for="(item, index) in bigFileList"
          :key="'bigFileList' + index"
        >
          {{ item.name }}
          <el-progress
            :stroke-width="2"
            :percentage="item.uploadProgressNum"
          ></el-progress>
        </div>
      </div>
      <!--正文 -->
        ...
        <div class="send-row-item-container">
          ...
          <div class="send-bottom">
            <div class="sender-info ellipsis">
              <!-- 发件人：<span class="sender-name">{{ senderInfo.account }}</span> -->
              <!-- &lt;{{ senderInfo.username }}&gt;｜ 签名：
              <div class="send-dropdown sign-text-row ellipsis">
                <DropDown
                  :initActiveName="signId"
                  :option="signList"
                  prefix=""
                  @selectChange="handleSignChange"
                />
              </div> -->
              <div class="send-bottom-box">
                ...
                <el-checkbox
                  ...
                  >定时发送</el-checkbox
                >
                <span class="cursor" @click="handleChangeTime"
                  >{{ form.timing }}</span
                >
                  <!-- <el-date-picker
                    v-model="form.timing"
    ...
    >
      ...
      <div style="display: flex; flex-direction: column">
        ...
        <div class="warning-footer">
          ...
          <el-button type="primary" @click="handleRangeTimeSubmit"
            >确定</el-button
          >
        </div>
      </div>
...
import MainBasicContainer from "@/components/MainBasicContainer";
...
import { queryEmail, uploadFile } from "../api/file";
  ...
  methods: {
      ...
      getMailDetails(query)
        .then(async (res) => {
          if (res.data.code === 0) {
            ...
            this.loading = false;
            const { mailBody, MailExtendList, MailRecipientList, MailContent } =
              resData;
            ...
            // 如果为文本类型，转换
            if (
              MailContent.textType.toLowerCase().indexOf("text/plain") !== -1
            ) {
            ...
            <div>
              ...
              <div style="background: #f4f4f4;padding: 15px 5px;margin-bottom: 15px;font-size: 19px;">
                <div style="font-weight: 500; color: #222222; line-height: 28px;margin: 0;">${
                  mailBody.subject
                }</div>
                <div>
                  <div style="font-size: 14px; font-weight: 400; color: #222222; line-height: 22px;">发件人：<span style="color: #BFBFBF;font-size: 14px;">${
                    mailBody.from
                  };</span></div>
                </div>
                <div>
                  <div style="font-size: 14px; font-weight: 400; color: #222222; line-height: 22px;">收件人：<span style="color: #BFBFBF;font-size: 14px;">${this.formatReplyRecipient(
                    action,
                    resData,
                    "sjr"
                  )}</span></div>
                ...
                ${
                  csArrStr
                    ? `<div><div style="font-size: 14px; font-weight: 400; color: #222222; line-height: 22px;">抄&nbsp;&nbsp;&nbsp;&nbsp;送：<span style="color: #BFBFBF;font-size: 14px;">${csArrStr}</span></div></div>`
                    : ""
                }
                ${
                  mailBody.sentDate
                    ? `<div> <div style="font-size: 14px; font-weight: 400; color: #222222; line-height: 22px;">时&nbsp;&nbsp;&nbsp;&nbsp;间：<span style="color: #BFBFBF;font-size: 14px;">${this.timerFormat(
                        mailBody.sentDate
                      )}</span></div></div>`
                    : ``
                }
          ...
          `
            ...
            const replacedData = replaceCIDImg(
              MailExtendList,
              content,
              "text/html",
              false,
    ...
    handleSearchUserMail(search) {
      ...
      getMailConfigList().then((res) => {
        ...
        if (res.code === 0) {
          let searchOptions = [];
          res.data.forEach((item) => {
            const str = `${item.account || ""}<${item.username}>`;
            searchOptions.push({
              label: str,
              value: item.username,
            });
    ...
    formatRecipientList(action, resData) {
      ...
      if (action === "huiFu") {
        ...
        if (mailFormat) {
          to = [
            { mailName: mailBody.from, mail: mailFormat, name: mailBody.from },
          ];
        }
        ...
        // 包括发送人
        const mailFormat = getMailStr(mailBody.from);
        cc = MailRecipientList.filter(
          (item) => item.mailType.value === "cc" && item.mail !== mailFormat
        );
    ...
    async handleSendMail(type, cb = null) {
      ...
      console.log(this.editor.txt.html(), '发送');
    ...
    onRemove(file, fileList) {
      ...
      // 过滤 url
      this.form.multipartFiles = this.form.multipartFiles.filter(
        (item) => item.url !== url
      );
    },
    onProgress(num, file) {
      let bigFileList = this.bigFileList;
      let filesArr = bigFileList.filter(
    ...
    onSuccess(fileName) {
      this.bigFileList = this.bigFileList.filter(
...
.send-header {
  ...
  .writeMailHeaderRight {
    ...
    .writeMailHeaderItem {
      ...
      .writeMailHeaderActionIcon {
        width: 14px;
        height: 14px;
        margin-right: 6px;
      }
      .writeMailHeaderActionText {
        font-size: 14px;
        font-weight: 400;
        color: #222222;
      }
...
.send-bottom {
  .sender-info {
    ...
    .el-dropdown, .el-checkbox__label {
      color: #858585;
    }
    .el-dropdown {
      padding: 4px 2px;
